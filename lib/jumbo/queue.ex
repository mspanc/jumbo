defmodule Jumbo.Queue do
  ...
  def handle_info({:DOWN, job_ref, :process, _job_pid, :normal}, %QueueState{running_jobs: running_jobs, logger_tag: logger_tag} = state) do
    %RunningJob{id: id, pid: job_pid} = running_jobs |> RunningJobsRegistry.get_by_ref(job_ref)
    info(logger_tag, "Job #{JobId.to_string(id)} #{inspect(job_pid)}: Process stopped gracefully")
  ...
  def handle_info({:DOWN, job_ref, :process, _job_pid, :killed}, %QueueState{running_jobs: running_jobs, logger_tag: logger_tag} = state) do
    %RunningJob{id: id, pid: job_pid} = running_jobs |> RunningJobsRegistry.get_by_ref(job_ref)
    warn(logger_tag, "Job #{JobId.to_string(id)} #{inspect(job_pid)}: Process has been killed")
  ...
  def handle_info({:DOWN, job_ref, :process, _job_pid, {{:nocatch, caught}, stacktrace}}, %QueueState{running_jobs: running_jobs, logger_tag: logger_tag} = state) do
    %RunningJob{id: id, pid: job_pid} = running_jobs |> RunningJobsRegistry.get_by_ref(job_ref)
    warn(logger_tag, "Job #{JobId.to_string(id)} #{inspect(job_pid)}: Thrown #{inspect(caught)} (#{inspect(stacktrace)})")
  ...
  def handle_info({:DOWN, job_ref, :process, _job_pid, {exception, stacktrace}}, %QueueState{running_jobs: running_jobs, logger_tag: logger_tag} = state) do
    %RunningJob{id: id, pid: job_pid} = running_jobs |> RunningJobsRegistry.get_by_ref(job_ref)
    warn(logger_tag, "Job #{JobId.to_string(id)} #{inspect(job_pid)}: Raised #{inspect(exception)} (#{inspect(stacktrace)})")
  ...
  defp do_enqueue_job(%FailedJob{id: job_id, module: job_module, args: job_args, failure_count: failure_count}, %QueueState{logger_tag: logger_tag} = state) do
    info(logger_tag, "Re-enqueueuing failed job #{JobId.to_string(job_id)}: #{job_module} (#{inspect(job_args)})")
  ...
  defp do_enqueue_job(%PendingJob{id: job_id, module: job_module, args: job_args, failure_count: failure_count}, %QueueState{logger_tag: logger_tag} = state) do
    info(logger_tag, "Re-enqueueuing pending job #{JobId.to_string(job_id)}: #{job_module} (#{inspect(job_args)})")
    do_enqueue_job(job_id, job_module, job_args, failure_count, state)
  end
  ...
  defp do_enqueue_job(job_module, job_args, %QueueState{running_jobs: running_jobs, pending_jobs: pending_jobs, failed_jobs: failed_jobs, logger_tag: logger_tag} = state) do
    ...
    info(logger_tag, "Enqueueuing new job #{JobId.to_string(job_id)}: #{job_module} (#{inspect(job_args)})")
  ...
  defp do_enqueue_job(job_id, job_module, job_args, job_failure_count, %QueueState{supervisor: supervisor, mode: mode, job_interval: job_interval, concurrency: concurrency, running_jobs: running_jobs, pending_jobs: pending_jobs, logger_tag: logger_tag} = state) do
    ...
    cond do
      ...
      true ->
        info(logger_tag, "Enqueued job #{JobId.to_string(job_id)}: #{job_module} (#{inspect(job_args)})")
  ...
  defp do_cleanup_job(job_ref, %QueueState{running_jobs: running_jobs, logger_tag: logger_tag} = state) do
    ...
    info(logger_tag, "Cleaning up Job #{JobId.to_string(id)} that was running as #{inspect(job_pid)}: #{job_module} (#{inspect(job_args)})")
  ...
  defp do_fail_job(job_ref, reason, info, stacktrace, %QueueState{running_jobs: running_jobs, failed_jobs: failed_jobs, logger_tag: logger_tag, max_failure_count: max_failure_count} = state) do
    ...
    warn(logger_tag, "Failing Job #{JobId.to_string(id)} that was running as #{inspect(job_pid)}, failure #{new_failure_count}/#{max_failure_count}: #{job_module} (#{inspect(job_args)})")
    ...
    cond do
      ...
      true ->
        warn(logger_tag, "Removing Job #{JobId.to_string(id)} that was running as #{inspect(job_pid)} due to too many failures (#{max_failure_count}): #{job_module} (#{inspect(job_args)})")
        %{state |
          running_jobs: running_jobs |> RunningJobsRegistry.delete_by_ref(job_ref)
        } |> do_next_job()
    end
  ...
  defp do_finalize_job(job_module, started_at, job_id, mode, job_interval) do
    ...
    duration = System.convert_time_unit((stopped_at - started_at), :native, :milliseconds)
